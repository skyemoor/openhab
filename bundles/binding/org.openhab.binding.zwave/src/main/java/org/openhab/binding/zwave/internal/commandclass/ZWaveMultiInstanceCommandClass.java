...
public class ZWaveMultiInstanceCommandClass extends ZWaveCommandClass {
		...
		switch (version) {
			...
			default:
				logger.warn(String.format("Unknown version %d for command class %s (0x%02x)", 
						version, this.getCommandClass().getLabel(), this.getCommandClass().getKey()));
				return; // don't set unknown version, revert to 1.
		}
	 ...
	 */
	public ZWaveEndpoint getEndpoint(int endpointId) throws IllegalArgumentException{
		
		if (!this.endpoints.containsKey(endpointId))
			throw new IllegalArgumentException(String.format("Endpoint with endpointId {} not found.", endpointId)); 
	...
	public void handleApplicationCommandRequest(SerialMessage serialMessage,
		...
		logger.debug("Handle Message Multi-instance/Multi-channel Request");
	...
	private void handleMultiInstanceReportResponse(SerialMessage serialMessage,
			int offset) {
		logger.debug("Process Multi-instance Report");
		int commandClassCode = serialMessage.getMessagePayload()[offset] & 0xFF;
		int instances = serialMessage.getMessagePayload()[offset + 1] & 0xFF;

		if (instances == 0)
			return; // the command class does not support multiple instances.
		...
		try {
			...
			logger.error(String.format("Unsupported commandClass (0x%02x) encountered.", commandClassCode));
		}
	}
	...
	private void handleMultiInstanceEncapResponse(
		...
		logger.debug("Process Multi-instance Encapsulation");
		int instance = serialMessage.getMessagePayload()[offset] & 0xFF;
	...
	private void handleMultiChannelEndpointReportResponse(
		...
		boolean changingNumberOfEndpoints = (serialMessage.getMessagePayload()[offset] & 0x80) != 0;
		endpointsAreTheSameDeviceClass = (serialMessage.getMessagePayload()[offset] & 0x40) != 0;
		int endpoints = serialMessage.getMessagePayload()[offset + 1] & 0x7F;
		...
		for (int i=1; i <= endpoints; i++) {
			ZWaveEndpoint endpoint = new ZWaveEndpoint(this.getNode(), i);
	...
	private void handleMultiChannelCapabilityReportResponse(SerialMessage serialMessage, int offset) {
		...
		int receivedEndpointId = serialMessage.getMessagePayload()[offset] & 0x7F;
		boolean dynamic = ((serialMessage.getMessagePayload()[offset] & 0x80) != 0);
		int genericDeviceClass = serialMessage.getMessagePayload()[offset + 1] & 0xFF;
		int specificDeviceClass = serialMessage.getMessagePayload()[offset + 2] & 0xFF;
		...
		for (int endpointId = startId; endpointId <= endId; endpointId++) {
			...
			for (int i = 0; i < serialMessage.getMessagePayload().length - offset - 3; i++) {
				int data = serialMessage.getMessagePayload()[offset + 3 + i] & 0xFF;
				if(data == 0xef )  {
	...
	private void handleMultiChannelEncapResponse(
			SerialMessage serialMessage, int offset) {
		logger.debug("Process Multi-channel Encapsulation");
		CommandClass commandClass;
		ZWaveCommandClass zwaveCommandClass;
		int endpointId = serialMessage.getMessagePayload()[offset] & 0xFF;
		int commandClassCode = serialMessage.getMessagePayload()[offset + 2] & 0xFF;
		...
		try {
			...
			logger.debug(String.format("Node %d, Endpoint = %d, calling handleApplicationCommandRequest.", this.getNode().getNodeId(), endpoint));
